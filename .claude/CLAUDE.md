...
- **`prompt-service/`** — Spring Boot REST service (port 8082): stateless, validates the JWT on every request via `JwtAuthFilter`, exposes `GET /api/hello`.
- **`web-client/`** — Angular 17 SPA (port 4200): initiates OAuth2 flows by navigating to `spring-auth`, captures the token from the callback URL, stores it in `localStorage`, and calls `prompt-service` with it as a Bearer token.
...
5. `HelloComponent` calls `prompt-service` at `http://localhost:8082/api/hello` with `Authorization: Bearer <token>`.
6. `JwtAuthFilter` in `prompt-service` validates the token and populates `UserPrincipal` (email + name) into the security context.
...
- Never modify the commit message body to include any AI tool attribution
- Keep commit messages clean with only the conventional commit format

## Commit Messages Format
- No trailer lines
- No co-authors
- No "Generated by" or "AI-assisted" footers